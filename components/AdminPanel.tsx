    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-1">Manage all exams and tasks.</p>
        </div>
        <button 
      ...
      {isAdding && (
        <form ref={formRef} onSubmit={handleSubmit} className="bg-white p-6 md:p-12 rounded-[3rem] border border-slate-100 shadow-2xl space-y-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-black text-slate-900">{editingId ? 'Edit Task' : 'New Task'}</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <label className="text-xs font-black text-slate-400 uppercase flex items-center gap-2"><Type size={14}/> Title</label>
              <input 
                required
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none font-bold focus:border-indigo-500 transition-all"
            ...
            <div className="space-y-3">
              ...
              <select 
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none font-black cursor-pointer"
            ...
            <div className="space-y-3">
              <label className="text-xs font-black text-slate-400 uppercase flex items-center gap-2"><CalendarIcon size={14}/> Date</label>
              <input 
                type="date"
                required
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none font-black cursor-pointer"
            ...
            <div className="space-y-3">
              <label className="text-xs font-black text-slate-400 uppercase flex items-center gap-2"><Clock size={14}/> Time (24h Format)</label>
              <input 
                type="time"
                required
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none font-black cursor-pointer"
            ...
            <div className="space-y-3">
              <label className="text-xs font-black text-slate-400 uppercase flex items-center gap-2"><MapPin size={14}/> Location</label>
              <input 
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 outline-none font-black"
          ...
          <div className="space-y-3">
            <label className="text-xs font-black text-slate-400 uppercase flex items-center gap-2"><AlertCircle size={14}/> Notes</label>
            <textarea 
              rows={4}
            ...
            {items.map(item => (
              <tr key={item.id} className={`hover:bg-slate-50 transition-colors group ${editingId === item.id ? 'bg-amber-50/50' : ''}`}>
                ...
                <td className="px-8 py-5 text-xs font-bold text-slate-400 uppercase text-start">
                  {subjects.find(s => s.id === item.subjectId)?.name[lang]}
                </td>
                ...
                <td className="px-8 py-5 text-end">
                  <div className="flex justify-end gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(item)} className="p-3 text-slate-400 hover:text-amber-600 hover:bg-white rounded-xl shadow-sm transition-all border border-slate-100 hover:border-amber-200">
                      <Edit2 size={18}/>
                    </button>
                    <button onClick={() => deleteItem(item.id)} className="p-3 text-slate-400 hover:text-red-600 hover:bg-white rounded-xl shadow-sm transition-all border border-slate-100 hover:border-red-200">
                      <Trash2 size={18}/>
                    </button>
                  </div>
                </td>
